...
import { DuckDuckGoEmailService, MoEmailService, TempMailPlusService, GmailIMAPAccount } from './email-service'
...
// Kiro IDE's own client_id (Authorization Code + PKCE flow)
// This is what Kiro IDE uses — accounts registered this way look like real IDE logins
const KIRO_CLIENT_ID = 'hUM3Hw6IaEzhXkzczAB3anVzLWVhc3QtMQ'
...
async function waitForSelector(win: BrowserWindow, selector: string, timeoutMs = 30000): Promise<boolean> {
...
async function clickWithCookieDismiss(win: BrowserWindow, selector: string, timeoutMs = 10000): Promise<boolean> {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    const stillThere = await win.webContents.executeJavaScript(
      `!!document.querySelector(${JSON.stringify(selector)})`
    ).catch(() => false)
...
async function typeInto(win: BrowserWindow, selector: string, text: string, timeoutMs = 15000): Promise<boolean> {
  if (!await waitForSelector(win, selector, timeoutMs)) return false
...
export class BrowserRegistrar {
  ...
  private async createEmailAddress(): Promise<string> {
    if (this.cfg.useDDG) {
      if (!this.cfg.ddgAuthToken || !this.cfg.ddgGmailEmail) throw new Error('DDG config incomplete')
   ...
   */
  private waitForAuthCode(win: BrowserWindow, redirectUri: string, state: string, timeoutMs = 300000): Promise<string> {
    return new Promise((resolve, reject) => {
      ...
      const onFail = (_: unknown, errorCode: number, _errorDesc: string, validatedURL: string): void => {
  ...
  private async stepConfirmDevice(win: BrowserWindow): Promise<void> {
    // "Allow access" button may appear: data-testid="allow-access-button"
    const allowAppeared = await waitForSelector(win, 'button[data-testid="allow-access-button"]', 10000)
    if (allowAppeared) {
      this.log('[Browser] Clicking Allow access')
      await clickWithCookieDismiss(win, 'button[data-testid="allow-access-button"]')
      await waitForPageLoad(win, 2000)
    }
  }
  ...
  async run(): Promise<RegistrationResult> {
    ...
    const state = require('crypto').randomBytes(16).toString('hex') as string
    ...
    const authURL = this.buildAuthURL(challenge, state, redirectUri)
    this.log(`[Browser] [1] Auth URL built (PKCE, client_id: ${KIRO_CLIENT_ID.slice(0, 8)}...)`)
    ...
    try {
      ...
      await win.loadURL(authURL).catch(() => { /* ERR_ABORTED on redirects is normal */ })
      this.checkAborted()
      ...
      const tokenData = await this.exchangeCodeForTokens(authCode, verifier, redirectUri)
